...
const getTimeZone = (hass: HomeAssistant): string => {
  const localeTimeZone = (hass as HomeAssistantWithLocaleTimeZone).locale
    .time_zone;

  return localeTimeZone === TimeZone.server
...
export const checkTimeInRange = (
  ...
  if (weekdays && weekdays.length > 0) {
    const currentWeekday = WEEKDAY_MAP[now.getDay() as WeekdayIndex];
